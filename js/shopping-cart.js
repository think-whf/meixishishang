	...
	if(cookie){
		...
		for(var j=0;j<arr.length;j++){
			for(var k=0;k<newArr.length;k++){
				if(newArr[k].indexOf("shopping=")!=-1){
					var shopArr;
					var shop;
					var shopping;

					if(k==0){
						shopArr = newArr[k].substring("shopping=".length);
					}
					else{
						shopArr = newArr[k].substring("shopping=".length+1);
					}					
		...
		function cart(){
			var $cart = $(".cart");
				...
				var total = $("<div/>").html("总价 : "+userInfo.money+"<button>立即购买</button>").addClass("cartTotal");
